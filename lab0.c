...
typedef enum StateTypeEnum {ButtonPress, WaitForRelease, LEDToggle} stateType;
// ******************************************************************************************* //
...
int main(void)
{
	...
	// 0 .
	LATB = 0xFFFF;              //All LEDS are off at start
	...
	TRISBbits.TRISB12 = 0;      //RB12 is output
    
	// SW1 of the 16-bit 28-pin Starter Board is connected to pin RB 5.
	...
	while(1)
	{
        ...
        switch(state) {
            case ButtonPress:
                if (PORTBbits.RB5==0) {     //if button press:
                    PR1 = 14400/2;          //blink twice as fast
                    TMR1=0;                 //reset timer
                    state = LEDToggle;      //change states
                }
                break;
            case WaitForRelease:
                if (PORTBbits.RB5 == 1) {   //if button released:
                    PR1 = 14400;            //reset period register (blink normal)
                    TMR1 = 0;               //reset timer
                    state = WaitForPress;   //change states
                }
                break;
            case LEDToggle:
                LATB = LATB^0x8000;         //toggle output values to turn LED on/off
                state=WaitForRelease;       //check for button release
                LATBbits.LATB15 = 1;        //keep LED 15 off
                break;
        }
        
		// Use the UART RX interrupt flag to wait until we recieve a character.
		if(IFS0bits.U1RXIF == 1) {
            LATB = 0xFFFF;      //LEDs off
